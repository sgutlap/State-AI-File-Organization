...
# Capture invoker CWD before chdir so relative folder args resolve correctly
# when this entrypoint is invoked from a repo root above Baselines/old/v1.
_INVOKE_CWD = Path.cwd()
os.chdir(ROOT)
...
def main():
    ...
    p.add_argument("--ckpt", default=str(ROOT / "artifacts" / "model"))
    ...
    if not folder.is_absolute():
        folder = (_INVOKE_CWD / folder).resolve()
    else:
    ...
    if not ckpt.is_absolute():
        # Prefer invoker-relative, then V1 ROOT-relative (post-relocation defaults).
        inv = (_INVOKE_CWD / ckpt).resolve()
        ckpt = inv if inv.exists() else (ROOT / ckpt).resolve()
    else:
        ckpt = ckpt.resolve()
    args.ckpt = str(ckpt)
