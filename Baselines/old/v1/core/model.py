...
class StudentNet(nn.Module):
    def __init__(self, config, num_classes, tabular_embed_dim=64, dropout=0.1):
        super().__init__()
        # from_config = architecture only, no hub download (weights come from model.pt)
...
class Student:
    def __init__(self, taxonomy=None, base_model=BASE_MODEL, ckpt: str | Path | None = None):
        ...
        try:
            ...
            raise RuntimeError(
                ...
                "Fix: ensure artifacts/model/ has config.json + tokenizer files,\n"
                "or run once online so Hugging Face can cache distilbert-base-uncased.\n"
                f"Original error: {exc}"
                
            ) from exc
        self.model.to(self.device)
        if ckpt_path and (ckpt_path / "model.pt").exists():
    ...
    def load(self, path: str):
        weight = Path(path) / "model.pt"
        if not weight.exists():
            raise FileNotFoundError(
                f"missing weights: {weight}\n"
                "model.pt is ~250MB and gitignored — copy it into artifacts/model/"
            )
        try:
